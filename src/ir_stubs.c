...
size_t ir_recv_poll(uint8_t *buf, size_t maxlen) {
    ...
    /* BANCO: simula el muestreo del FIFO del SC16IS750 de PokeStride.
     * PWDBG_FIFO_CAP=N limita a N bytes por poll, igual que un drenaje de FIFO
     * cada ~1.1 ms -> un paquete grande se reparte en varios polls y el firmware
     * ve gaps FALSOS a mitad de paquete. Reproduce la patologia del 3DS. */
    {
        static int fifo_cap = -1;
        if (fifo_cap < 0) {
            const char *e = getenv("PWDBG_FIFO_CAP");
            fifo_cap = (e && *e) ? atoi(e) : 0;
        }
        if (fifo_cap > 0 && maxlen > (size_t)fifo_cap) maxlen = (size_t)fifo_cap;
    }
...
/* Equivalente de banco: no queda ningun chunk pendiente = el aire esta callado.
 * (En el 3DS lo determina el nivel del FIFO; ver source/ir.c) */
bool ir_rx_air_silent(void) { return chunk_count == 0; }

/* El banco no tiene FIFO fisico ni eco: nada que drenar. */
