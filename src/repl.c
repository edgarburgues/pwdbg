...
/* --- pacing a tiempo real (portado de projects/pwdbg/src/run.c) ----------
 * Mantiene el tiempo emulado (totalCycles / 3.6864 MHz) a la par del reloj de
 * pared. Sin esto el REPL corre ~75x, la ventana de silencio de ~98 ms del
 * firmware se agota en ~1.3 ms reales y un peer externo por socket nunca llega
 * a responder. Se re-ancla en cada `run` para que las esperas de stdin no
 * provoquen una rafaga de recuperacion. */
#define PACE_H8_HZ 3686400ULL
static bool     pace_enabled  = false;
static uint64_t pace_start_ns = 0;

static uint64_t pace_now_ns(void) {
    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
    return (uint64_t)ts.tv_sec * 1000000000ULL + (uint64_t)ts.tv_nsec;
}
static uint64_t pace_cycles_ns(uint64_t cyc) {   /* overflow-safe */
    return (cyc / PACE_H8_HZ) * 1000000000ULL +
           ((cyc % PACE_H8_HZ) * 1000000000ULL) / PACE_H8_HZ;
}
...
static StepResult step_until(ReplState *st,
    ...
    if (pace_enabled)                      /* re-ancla: ver pace_* arriba */
        pace_start_ns = pace_now_ns() - pace_cycles_ns(st->totalCycles);
    ...
    while (!stopFlag) {
        ...
        /* throttle: solo cada 256 instrucciones y solo si vamos >150us
         * adelantados, para que el coste de nanosleep no domine. */
        if (pace_enabled && (st->totalInstrs & 0xFF) == 0) {
            uint64_t target_ns = pace_start_ns + pace_cycles_ns(st->totalCycles);
            uint64_t cur_ns    = pace_now_ns();
            if (target_ns > cur_ns + 150000ULL) {
                uint64_t d = target_ns - cur_ns;
                struct timespec req = { (time_t)(d / 1000000000ULL),
